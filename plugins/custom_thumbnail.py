...
async def generate_custom_thumbnail(bot, update):
    ...
    if update.reply_to_message is not None (update.from_user.id, update.text, "generatecustomthumbnail"):
        ...
        if reply_message.media_group_id is not None:
            download_location = Config.DOWNLOAD_LOCATIONS + "/" + str(update.from_user.id) + "/" + str(reply_message.media_group_id) + "/"
...
async def savethumbnail(bot, update):
    ...
    if update.reply_to_message.media_group_id is not None:
        # album is sent
        download_location = Config.DOWNLOAD_LOCATIONS + "/" + str(update.from_user.id) + "/" + str(update.media_group_id) + "/"
        ...
        download_location = Config.DOWNLOAD_LOCATIONS + "/" + str(update.from_user.id) + ".jpg"
        await bot.download_media(
            message=update.reply_to_message,
            file_name=download_location
        )
...
async def delete_thumbnail(bot, update):
    ...
    download_location = Config.DOWNLOAD_LOCATIONS + "/" + str(update.from_user.id)
    try:
