...
async def savethumbnail(bot, update):
    ...
    if update.reply_to_message.media_group_id is not None:
        # album is sent
        download_location = Config.DOWNLOAD_LOCATION + "/" + str(update.reply_to_message.user.id) + "/" + str(update.media_group_id) + "/"
        ...
        download_location = Config.DOWNLOAD_LOCATION + "/" + str(update.reply_to_message.user.id) + ".jpg"
        await bot.download_media(
            message=update,
            file_name=download_location
        )
